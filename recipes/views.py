...
#                 # Rename columns
#                 # recipes_df.columns = ['Recipe no.', 'ID', 'Recipe Name', 'Cooking Time', 'Ingredients', 'Difficulty', 'Picture']

#                 # Generate a chart based on the selected chart type
#                 chart = get_chart(chart_type, recipes_df, labels=recipes_df['name'].values)

#                 # chart = get_chart(chart_type, recipes_df, labels=recipes_df['Recipe Name'].values)
...
def records(request):
    ...
    return render(request, 'recipes/records.html', {'form': form, 'recipes_df': recipes_df, 'chart': chart})
